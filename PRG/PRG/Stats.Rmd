...
6. [Correlations](#correlations)  
   ...
   6.2. [Movements](#movements)  
   6.3. [Patients symptoms](#patients-symptoms)
...
unique_participants$Regions = sprintf("%02d", as.integer(unique_participants$Regions))
unique_participants = unique_participants %>%
  mutate(Region_Name = region_names[Regions])
participants_by_region = unique_participants %>%
  group_by(Region_Name) %>%
  summarise(Nombre_Participants = n()) %>%
  mutate(Pourcentage = round(Nombre_Participants / sum(Nombre_Participants) * 100, 2)) %>%
  arrange(desc(Nombre_Participants))
...
```{r}
occupation_names = c(
  "01" = "Exploitants de l'agriculture, sylviculture, pêche et aquaculture",
  "02" = "Artisans",
  "03" = "Commerçants et assimilés",
  "04" = "Chefs d’entreprise de plus de 10 personnes",
  "05" = "Chômage",
  "06" = "Cadres administratifs et techniques de la fonction publique",
  "07" = "Conducteurs d’engins, caristes, magasiniers et ouvriers du transport",
  "08" = "Conducteurs de véhicules de transport, chauffeurs-livreurs, coursiers",
  "09" = "Cadres des services administratifs et commerciaux d’entreprise",
  "10" = "Ingénieurs et cadres techniques d’entreprise",
  "11" = "Professions de l’enseignement primaire et professionnel, de la formation",
  "12" = "Professions intermédiaires de la fonction publique (administration, sécurité)",
  "13" = "Professions intermédiaires de la santé et du travail social",
  "14" = "Professions libérales",
  "15" = "Professions intermédiaires administratives et commerciales des entreprises",
  "16" = "Professions de l’information, de l’art et des spectacles",
  "17" = "Agents de maîtrise (hors maîtrise administrative)",
  "18" = "Ministres du culte et religieux consacrés",
  "19" = "Ouvriers agricoles, des travaux forestiers, de la pêche et de l’aquaculture",
  "20" = "Employés de commerce",
  "21" = "Étudiants",
  "22" = "Personnels des services directs aux particuliers",
  "23" = "Ouvriers qualifiés de type industriel",
  "24" = "Ouvriers qualifiés de type artisanal",
  "25" = "Policiers, militaires, pompiers, agents de sécurité privée",
  "26" = "Employés administratifs d’entreprise",
  "27" = "Ouvriers peu qualifiés de type industriel",
  "28" = "Ouvriers peu qualifiés de type artisanal",
  "29" = "Employés administratifs de la fonction publique, agents de service et assimilés",
  "30" = "Techniciens",
  "31" = "Sans emploi",
  "32" = "Autre",
  "33" = "Sans emploi",
...
unique_participants$Occupation = sprintf("%02d", as.integer(unique_participants$Occupation))
unique_participants = unique_participants %>%
  mutate(Occupation_Name = occupation_names[Occupation])
participants_by_occupation = unique_participants %>%
  group_by(Occupation_Name) %>%
  summarise(Nombre_Participants = n()) %>%
  mutate(Pourcentage = round(Nombre_Participants / sum(Nombre_Participants) * 100, 2)) %>%
  arrange(desc(Nombre_Participants))
participants_by_occupation
```
...
ISZ only 
```{r}
patients_only = combined_impressions %>% 
  filter(Type == "Patient")

plot_ISZ = ggplot(patients_only, aes(x = Impression, y = Score, fill = Dataset)) +
  stat_summary(fun = mean, geom = "bar", position = position_dodge(width = 0.9)) +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", position = position_dodge(width = 0.9), width = 0.2) +
  theme_minimal() +
  labs(
    title = "First Impressions of Individuals with Schizophrenia Across Conditions",
    x = "First Impression",
    y = "Mean Score"
  ) +
...
```{r}
model = lmerTest::lmer(Total_score ~ interaction(Type, Dataset) * Age * Gender + (1|CASE), data = combined_results)
summary(model)
```
...
```{r}
correlation_data = result_1st_imp %>%
  dplyr::select(
    Total_score
  )
...
### 6.3 Patients symptoms
<a name="Patients symptoms"></a>

Correlations
```{r}
patient_data = result_1st_imp %>%
...
if (length(significant_vars) > 0) {
  ...
  patient_data_selected = result_1st_imp %>%
    filter(Type == "Patient") %>%
...
```
```{r}
au_vars = c("stimuli_AU26_r")

cat("\nGraphe de régression entre les variables significatives et AU26:\n")
for (var in selected_vars) {
  for (au in au_vars) {
    result = correlation_with_pvalue(patient_data_selected[[var]], patient_data_selected[[au]])
...
Correlations
```{r}
patient_data_PANSS = result_1st_imp %>%
  dplyr::filter(Type == "Patient") %>%
  dplyr::select(
    Total_score,  
